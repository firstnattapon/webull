...
| 5 | ดึง position + quote แล้วคำนวณ | ~500ms (Webull API) | `PASS_THRESHOLD` / ไปต่อ |
| 6 | มี open order ของ symbol นี้อยู่ไหม? | ~200ms (Webull API; เช็คเฉพาะเมื่อจะซื้อ/ขาย) | `PASS_OPEN_ORDER` / `OK` |
...
`quantity` ต้องมาจาก Account Positions ของ symbol เดียวกัน ห้ามแทนด้วย 0
เมื่อ response มี symbol แต่ไม่มี quantity เพราะจะทำให้ bot เข้าใจผิดว่าพอร์ตว่าง
และส่ง BUY เต็ม `FIX_C`
...
6. **Position schema ผิด/อ่าน quantity ไม่ได้** — ตอบ 502 แทนการเดาเป็น 0
7. **Open-order guard** — ถ้า order ก่อนหน้ายังไม่ fill จะตอบ `PASS_OPEN_ORDER` ไม่ส่ง order ซ้อน
